...
module.exports = {
    getEstadisticasAdministradorGrand: function (req, res, next) {
        ...
        db1.query("SELECT cen_id, cen_nombre FROM modelo_central INNER JOIN marca_central ON modelo_central.mar_id = marca_central.mar_id WHERE marca_central.mar_nombre = '" + req.query.central + "'", function (err, rows, fields) {
            if (err) {
                ...
                res.sendStatus(500);
                db1.end();
                IngresarError('Error', 'Buscar Modelo Central - Estadísticas Grandstream', err);
            } else {
                db1.end();
                ...
                db.query("SELECT DISTINCT YEAR(llamada.lla_fecha) AS 'anio' FROM llamada WHERE YEAR(llamada.lla_fecha) != 1999", function (err, rows2, fields) {
                    if (err) {
                        ...
                        res.sendStatus(500);
                        db.end();
                        IngresarError('Error', 'Buscar Años - Estadísticas Grandstream', err);
                    } else {

                        db.end();
                        res.render('administrador/adminEstadisticasGrand', {
    ...
    buscarResumenGlobalGrand: function (req, res, next) {
        ...
        db.query("CALL buscar_resumen_global_E_G('" + modelo_central + "')", function (err, rows, fields) {
            if (err) {
                ...
                res.sendStatus(500);
                db.end();
                IngresarError('Error', 'Buscar Resumen - Estadísticas Grandstream', err);
            } else {
                res.send(rows);
                db.end();
            }
    ...
    buscarTipoGlobalGrand: function (req, res, next) {
        ...
        db.query("CALL buscar_tipo_llamada_global_E_G('" + modelo_central + "')", function (err, rows, fields) {
            if (err) {
                ...
                res.sendStatus(500);
                db.end();
                IngresarError('Error', 'Buscar Tipo - Estadísticas Grandstream', err);
            } else {
                res.send(rows);
                db.end();
            }
    ...
    buscarTiempoGlobalGrand: function (req, res, next) {
        ...
        db.query("CALL buscar_tiempo_llamadas_mes_E_G('" + modelo_central + "'," + anio_busqueda + ")", function (err, rows, fields) {
            if (err) {
                ...
                res.sendStatus(500);
                db.end();
                IngresarError('Error', 'Buscar Tiempo - Estadísticas Grandstream', err);
            } else {
                res.send(rows);
                db.end();
            }
